...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans transition-colors duration-200">
      ...
      <header className="bg-[#161b22] dark:bg-[#0d1117] border-b border-[#30363d] py-3 px-6 shadow-sm">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3 group">
            <div className="bg-[#2ea043] p-1.5 rounded-md text-white transition-transform group-hover:scale-110">
          ...
          <div className="flex items-center gap-4">
            <a 
              ...
              className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-green-500 hover:text-green-400 transition-colors"
            >
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8">
        <div className="w-full max-w-md animate-fade-in-up">
          <div className="mb-10 text-center sm:text-left">
            <h1 className="text-4xl font-extrabold tracking-tight mb-2 text-green-600 dark:text-green-500">mshr.dev</h1>
            <p className="opacity-70 text-sm font-medium">Greater Boston Mesh link shortener.</p>
          </div>

          <div className="bg-white dark:bg-[#21262d] border border-[#d0d7de] dark:border-[#30363d] rounded-xl p-6 shadow-sm">
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div className="space-y-2 group">
                <label htmlFor="url" className="text-xs font-bold uppercase tracking-widest text-green-600 dark:text-green-500 group-focus-within:opacity-100 transition-opacity">Destination URL</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    className="w-full pl-9 pr-4 py-2.5 bg-transparent border border-[#d0d7de] dark:border-[#30363d] rounded-md focus:outline-none focus:border-green-500 dark:focus:border-green-500 focus:ring-1 focus:ring-green-500/30 transition-all text-sm"
                  />
                </div>
              </div>

              <div className="space-y-2 group">
                <label htmlFor="expiry" className="text-xs font-bold uppercase tracking-widest text-green-600 dark:text-green-500 group-focus-within:opacity-100 transition-opacity">Expiration</label>
                <div className="relative">
                  <select
                    id="expiry"
                    value={expiry}
                    onChange={(e) => setExpiry(e.target.value)}
                    className="w-full px-3 py-2.5 bg-transparent border border-[#d0d7de] dark:border-[#30363d] rounded-md focus:outline-none focus:border-green-500 dark:focus:border-green-500 focus:ring-1 focus:ring-green-500/30 transition-all appearance-none cursor-pointer text-sm"
                  >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 mt-2 bg-[#2ea043] dark:bg-[#238636] text-white hover:bg-[#2c974b] dark:hover:bg-[#2ea043] font-bold rounded-md transition-all active:scale-[0.98] disabled:opacity-50 disabled:active:scale-100 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm uppercase tracking-widest shadow-sm"
              >
                {loading ? (
                  <>
            ...
            {result && (
              <div className="mt-10 pt-8 border-t border-[#d0d7de] dark:border-[#30363d] animate-fade-in-up" style={{ animationDuration: '0.3s' }}>
                <div className="flex justify-between items-center mb-3">
                  <span className="text-xs font-bold uppercase tracking-widest text-green-600 dark:text-green-500">Success</span>
                  {result.expiryLabel && (
                    <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">
                      Exp: {result.expiryLabel}
                    </span>
                  )}
                </div>
                
                <div className="flex items-center justify-between border border-[#d0d7de] dark:border-[#30363d] rounded-md p-1 pl-3 bg-[#f6f8fa] dark:bg-[#0d1117] transition-all hover:border-green-500/50">
                  <span className="font-mono text-sm truncate pr-2 select-all font-bold text-green-600 dark:text-green-500">
                    {result.shortUrl}
                  </span>
                  <button
                    onClick={copyToClipboard}
                    className="flex items-center justify-center p-2.5 bg-white dark:bg-[#21262d] border border-[#d0d7de] dark:border-[#30363d] hover:bg-neutral-50 dark:hover:bg-neutral-800 active:scale-90 rounded transition-all text-green-600 dark:text-green-500"
